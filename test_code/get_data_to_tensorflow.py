	...
	for entry in range(len(dataset)):    # iterate over all images
		...
		for obj in range(len(dataset[entry][3])):    # over all objects in an image
			if dataset[entry][3][obj] in class_we_care:   # select only objects we interested in
				...
				label = class_we_care.index(label_str[0])  # use class_we_care as the values of labels
				try:
				...
				##### possibly resize croopped image?
				# Create a feature
		        feature = {
...
if __name__ == '__main__':
	...
	train = addrs[0:int(0.8*len(dataset))]
	test = addrs[int(0.8*len(dataset)):]

	# create tensorflow records
	createDataRecord('train.tfrecords', train)
	createDataRecord('test.tfrecords', test)
