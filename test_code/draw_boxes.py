...
def crop_image(im, anno):
    return patches.Rectangle([anno[0],anno[1]], anno[2], anno[3],linewidth=1,edgecolor='r',facecolor='none')
...
def create_data_record(dataset):
    ...
    for entry in [5000, 5001, 5002, 5003, 5020,7000, 6000]:    # iterate over all images
...
# Display the image
        ...
        for obj in range(len(dataset[entry][3])):    # over all objects in an image
            ...
            try:
                ...
                failed += 1
        plt.savefig('sun' +str(name_counter)+'.jpg')
                # if label_str == 'chair':
                #     chair_counter += 1
                # if label_str == 'door':
                #     door_counter += 1
